...
public static partial class IrcUtils{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)] public static string ToColor(this string str, byte color)=>color >= 16 ? $"{ColorChar}{str}" : $"{ColorChar}{color:00}{str}{ColorChar}";
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)] public static bool IsCtcp(this string str)=>str.StartsWith(CtcpChar) && str.EndsWith(CtcpChar);
	...
	public static string FormatName(this IrcUser user, byte configId, bool useHostmask = false){
		...
		if(user.Mode?.Contains('o') is true){
	...
	public static async Task<string> ConvertFormatting(string strToFormat, DiscordChannel channel){
		...
		if(strToFormat.Contains('@')){
			...
			if(strToFormat.Contains(EscapePrefix)){
				...
				for(int i = 0; i < messageCmd.Length; i++){
					...
					messageCmd[i] = messageCmd[i] switch{
						"last"=>LastUserToSpeak[channel].Peek().Mention,
						var _=>messageCmd[i][EscapePrefix.Length..]
					};
				}
		...
		if(!strToFormat.Contains(ColorChar)) return strToFormat;
