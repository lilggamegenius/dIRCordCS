...
namespace dIRCordCS.Utils {
    public static class LilGUtil {
        ...
        public static string formatFileSize(long size){
            ...
            if (t > 1){ hrSize = dec.format(t).concat(" TB"); }
            else if (g > 1){ hrSize = dec.format(g).concat(" GB"); }
            else if (m > 1){ hrSize = dec.format(m).concat(" MB"); }
            else if (k > 1){ hrSize = dec.format(k).concat(" KB"); }
            else{ hrSize = dec.format((double)size).concat(" B"); }
        ...
        public static string[] splitMessage(this string stringToSplit, int amountToSplit = 0, bool removeQuotes = true){
            ...
            while (argSep.find()) list.Add(argSep.group(1));
        ...
        public static bool MatchHostMask(string hostmask, string pattern){
            ...
            if (!wildCardMatch(nick, patternNick)) return false;
            if (!wildCardMatch(userName, patternUserName)) return false;
