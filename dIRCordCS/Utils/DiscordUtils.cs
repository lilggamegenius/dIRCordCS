...
public static partial class DiscordUtils{
	...
	public static string GetHostMask(this DiscordUser user){
		...
		return member != null ? $"{member.DisplayName}!{user.Username}@{user.Id}" : $"{user.Username}!{user.Username}@{user.Id}";
	}

	public static string FormatName(this DiscordMember user, byte configId, FormatAs format){
		return format switch{
			FormatAs.EffectiveName=>FormatName(user, configId),
			FormatAs.NickName=>FormatName(user, configId, user.Nickname),
			FormatAs.Username=>FormatName(user, configId, user.Username),
			FormatAs.Id=>FormatName(user, configId, Convert.ToString(user.Id)), // ????
			_=>string.Empty
		};
	}
	...
	private static string FormatRole(this DiscordRole role){
	...
	private static bool CanInteract(this DiscordRole issuer, DiscordRole target){
		if(issuer == null){
			throw new ArgumentNullException(nameof(issuer));
		}
...
public class ColorMappings{
	...
	public (byte, DiscordColor) this[Color color]=>ColorDictionary.TryGetValue(color, out (byte, DiscordColor) value) ? value : (byte.MaxValue, DiscordColor.None);
}
