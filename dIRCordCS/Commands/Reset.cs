...
namespace dIRCordCS.Commands{
	public class Reset : ICommand{
		...
		static Reset(){
			Bridge.RegisterCommand(nameof(Reset), new Reset());
		}
		...
		public async Task Help(IrcListener listener, IrcChannel channel, IList<string> args, PrivateMessageEventArgs e){
			await Bridge.Respond("This is used by the bot owner to reset the bot", channel);
		}
		...
		public void ResetBot(string message){
			throw new ResetException(message); // Trigger the reset
		}
	}
}
