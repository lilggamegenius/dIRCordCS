...
public class DiscordListener : Listener{
	...
	private async Task OnNewMessage(DiscordClient sender, MessageCreateEventArgs e){
		...
		try{
			...
			if(int.Parse(e.Author.Discriminator) != 0){
				Logger.Warn($"Unable to find Member for <{e.Author.Username}#{e.Author.Discriminator}> ({e.Author.Id}) in [{e.Guild.Name}] ({e.Guild.Id})");
			} else{
				Logger.Debug($"Unable to find Member for <{e.Author.Username}#{e.Author.Discriminator}> ({e.Author.Id}) in [{e.Guild.Name}] ({e.Guild.Id}). " +
							 "Most likely a webhook or some other non-real user");
			}
		}
	...
	private async Task OnGuildMemberAdded(DiscordClient sender, GuildMemberAddEventArgs e){
		...
		foreach(DiscordChannel channel in Program.Config.Servers[ConfigId].ChannelMapObj.Values){
			...
			await Bridge.GetChannel(this, channel).SendMessageAsync($"{member.FormatName(ConfigId)} [{member.Id}] has Joined #{channel.Name}");
		}
	}
	...
	private async Task OnGuildMemberRemoved(DiscordClient sender, GuildMemberRemoveEventArgs e){
		...
		foreach(DiscordChannel channel in Program.Config.Servers[ConfigId].ChannelMapObj.Values){
			...
			await Bridge.GetChannel(this, channel).SendMessageAsync($"{member.FormatName(ConfigId)} [{member.Id}] has quit #{channel.Name}");
		}
	}
	...
	private async Task OnGuildMemberUpdated(DiscordClient sender, GuildMemberUpdateEventArgs e){
		...
		DiscordMember member = e.Member;
		foreach(DiscordChannel channel in Program.Config.Servers[ConfigId].ChannelMapObj.Values){
			if(guild != channel.Guild) continue;
			await Bridge.GetChannel(this, channel)
						.SendMessageAsync($"*{member.FormatName(ConfigId, e.NicknameBefore).ToItalics()}* Has changed nick to {member.FormatName(ConfigId, e.NicknameAfter)}");
		}
	}
	...
	private static async Task OnSocketError(DiscordClient sender, SocketErrorEventArgs e)=>Logger.Error(e.Exception, e.Exception.Message);
}
